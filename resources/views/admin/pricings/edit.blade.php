...
<div class="card"><div class="card-body">
    ...
    <form action="{{ route('pricings.update', $pricing) }}" method="POST">@csrf @method('PUT')
        <div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label" for="price">Đơn giá (VNĐ) <span class="text-danger ms-1">*</span></label><input type="number" class="form-control" id="price" name="price" value="{{ old('price', (int) round((float) $pricing->price)) }}" min="0" step="100" required></div>
            ...
            <div class="col-md-6"><label class="form-label" for="status">Trạng thái <span class="text-danger ms-1">*</span></label><x-admin.status-select name="status" id="status" :options="\App\Enums\RecordStatus::options()" :selected="$pricing->status" class="form-select" required /></div>

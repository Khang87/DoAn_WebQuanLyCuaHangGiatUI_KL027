...
<div class="card"><div class="card-body">
    ...
    <form action="{{ route('pricings.store') }}" method="POST">@csrf
        <div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label" for="status">Trạng thái <span class="text-danger ms-1">*</span></label><select class="form-select" id="status" name="status" required><option value="active" @selected(old('status', 'active') === 'active')>Đang áp dụng</option><option value="inactive" @selected(old('status') === 'inactive')>Tạm ngừng</option></select></div>

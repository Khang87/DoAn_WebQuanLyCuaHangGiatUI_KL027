...
<div class="card"><div class="card-body">
    ...
    <form action="{{ route('deliveries.store') }}" method="POST">@csrf
        <div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label" for="status">Trạng thái</label><select class="form-select" id="status" name="status"><option value="pending" @selected(old('status', 'pending') === 'pending')>Chờ xác nhận</option><option value="picking" @selected(old('status') === 'picking')>Đang nhận đồ</option><option value="delivering" @selected(old('status') === 'delivering')>Đang giao đồ</option><option value="completed" @selected(old('status') === 'completed')>Hoàn thành</option><option value="cancelled" @selected(old('status') === 'cancelled')>Đã hủy</option></select></div>

...
<div class="delivery-form-shell"><div class="card"><div class="card-body">
    ...
    <form action="{{ route('deliveries.update', $delivery) }}" method="POST">@csrf @method('PUT')
        <div class="row g-3">
            ...
            <div class="col-12"><label class="form-label" for="status">Trạng thái</label><select class="form-select" id="status" name="status"><option value="pending" @selected(old('status', $delivery->status) === 'pending')>Chờ xác nhận</option><option value="picking" @selected(old('status', $delivery->status) === 'picking')>Đang nhận đồ</option><option value="delivering" @selected(old('status', $delivery->status) === 'delivering')>Đang giao đồ</option><option value="completed" @selected(old('status', $delivery->status) === 'completed')>Hoàn thành</option><option value="cancelled" @selected(old('status', $delivery->status) === 'cancelled')>Đã hủy</option></select></div>
